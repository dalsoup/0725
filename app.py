...
def get_weather_from_ultra_fcst(region_name):
    ...
    temp = float(df.loc["T1H"]["fcstValue"]) if "T1H" in df.index else None
    wind = float(df.loc["WSD"]["fcstValue"]) if "WSD" in df.index else None
...
if st.session_state.show_inputs:
    with st.container():
        ...
        use_api = st.checkbox("기상청 API 자동 불러오기", key="api_checkbox")

        if use_api:
            weather_data = get_weather_combined(region) or {}
        else:
            weather_data = {}
